...
public final class DiscordPresenceViewer extends LauncherComponent {
    ...
    public void initialize() throws IOException {
        ...
        ThreadUtil.runDaemon(() -> {
            try {
                ...
                DiscordEventHandlers handlers =
                    new DiscordEventHandlers.Builder()
                        .setReadyEventHandler(user ->
                            LOG.info("successfully connected to discord client")
                        )
                        .setErroredEventHandler((code, message) -> {
                            LOG.severe(
                                "error occurred in Discord RPC: #" +
                                    code +
                                    ": " +
                                    message
                            );
                ...
                LOG.logp(
                    Level.SEVERE,
                    "io.github.spookylauncher.components.DiscordPresenceViewer",
                    "initialize",
                    "Throw!",
                    e
                );
    ...
    public void showMenuPresence() {
        ...
        DiscordRPC.discordUpdatePresence(
            new DiscordRichPresence.Builder(
                components.get(Translator.class).getLocale().get("inMenu")
            )
    ...
    public void showGamePresence(VersionInfo version) {
        LOG.info(
            "switching to game rich presence (version: " + version.name + ")"
        );

        Locale locale = components.get(Translator.class).getLocale();

        String nickname = components
