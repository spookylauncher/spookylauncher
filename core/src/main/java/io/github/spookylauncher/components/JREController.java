...
public final class JREController extends LauncherComponent {
    ...
    private JREsManifest getManifest() {
        return (
            (ManifestDownloader<JREsManifest>) components.get(
    ...
    public void checkJre() {
        ...
        final OptionsController optionsController = components.get(
    ...
    public JreInfo[] findJres() {
        if (foundJREs == null) {
            ...
            if (dirs != null) {
                for (File dir : dirs)
                    jres.add(getManifest().findJreInfo(dir.getName()));
            }
            ...
            for (String javaPath : jresPaths) {
                ...
                try {
                    props = new FileCollector(
                        new File(javaDir, "release")
                    ).collectProperties();
                } catch (IOException e) {
                    LOG.severe(
                        "failed to read info of jre \"" + javaPath + "\""
                    );
                    LOG.logp(
                        Level.SEVERE,
                        "io.github.spookylauncher.components.JREController",
                ...
                jre.fullVersion = jre.fullVersion.substring(
                    1,
                    jre.fullVersion.length() - 1
                );

                try {
                    String[] splits = jre.fullVersion.split("\\.");

                    jre.majorVersion = Integer.parseInt(
                        splits[0].equals("1") ? splits[1] : splits[0]
                    );
                } catch (Exception e) {
                    LOG.logp(
                        Level.SEVERE,
    ...
    public File getJreDirectory(JreInfo info) {
        if (info instanceof ExternalJreInfo) return new File(
            ((ExternalJreInfo) info).path
        )
            .getParentFile()
            .getParentFile();
        ...
        try {
            ...
            LOG.severe(
                "failed to delete tree \"" +
                    getJreDirectory(info).getAbsolutePath() +
                    "\""
            );
            LOG.logp(
                Level.SEVERE,
    ...
    public boolean selectJre(JreInfo info) {
        if (info instanceof ExternalJreInfo) return selectExternalJre(
            ((ExternalJreInfo) info).path
        );
    ...
    public JreInfo getSelectedJre() {
        ...
        switch (options.selectedJavaType) {
            case CUSTOM:
                return new ExternalJreInfo(options.customJavaPath);
            case EXTERNAL:
                return new ExternalJreInfo(options.selectedJavaPath);
            default:
                return getManifest().findJreInfo(options.selectedJava);
        }
    }
    ...
    public File getJreExecutable(JreInfo info) {
        ...
        return IOUtils.find(
            "bin/" + (OSType.CURRENT == OSType.WINDOWS ? "java.exe" : "java"),
            getJreDirectory(info)
        );
    }

    public void installJre(JreInfo info, Consumer<Boolean> onInstalled) {
        LOG.info(
            "start installing JRE " +
                info.fullVersion +
                ". Vendor: " +
                info.vendor +
                ", Major version: " +
        ...
        if (info.downloads.containsKey(OSType.CURRENT)) url =
            info.downloads.get(OSType.CURRENT);
        else {
            LOG.severe(
                "JRE installation failed because downloads are not available"
            );
            uiProvider
                .messages()
                .error(
                    ...
                    String.format(
                        locale.get("jreDownloadNotExists"),
                        info.fullVersion
                    )
                );
            return;
