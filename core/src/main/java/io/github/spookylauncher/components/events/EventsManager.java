...
public final class EventsManager extends LauncherComponent {
    ...
    public EventsManager(ComponentsController components) {
    ...
    public void emit(String event, Object... args) {
        ...
        for (Handler handler : events.get(event)) {
            try {
                ...
                LOG.severe(
                    "exception occurred in event handler \"" + handler + "\""
                );
                LOG.logp(
                    Level.SEVERE,
                    "io.github.spookylauncher.components.events.EventsManager",
                    "emit",
                    "Throw!",
                    e
                );
