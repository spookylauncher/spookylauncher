...
public final class VersionsList extends ManifestDownloader<VersionsManifest> {
    ...
    public boolean isInstalled(VersionInfo version) {
        ...
        for (String name : getInstalledVersions())
            if (name.equals(version.name)) return true;
    ...
    public VersionInfo getSelectedVersionInfo() {
        ...
        return manifest.getVersionInfo(
            repo,
            components.get(OptionsController.class).getOptions().selectedVersion
        );
    }
    ...
    public boolean downloadManifest() {
        if (super.downloadManifest()) {
            ...
            final Runnable task = () -> {
                if (
                    uiProvider.panel().getCurrentVersion() == null &&
                    components
