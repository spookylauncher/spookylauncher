...
public final class VersionsInstaller extends LauncherComponent {
    ...
    public void installLibraries(
        VersionInfo version,
        Consumer<Boolean> onInstalledCallback
    ) {
        LibrariesController libsController = components.get(
        ...
        AtomicInteger uninstalledCount = new AtomicInteger(
            version.libsCollections.length
        );
        ...
        for (String libs : version.libsCollections) {
            ...
            if (!libsController.isInstalled(libsCollection)) {
                libsController.install(libsCollection, s -> {
                    uninstalledCount.getAndDecrement();

                    if (!s) success.set(false);
                });
    ...
    public void install(VersionInfo version) {
        ...
        if (versions.isInstalled(version)) {
            ...
            uiProvider
                ...
                .warning(
                    ...
                    locale.get("versionAlreadyInstalled")
                );
            return;
        }
        ...
        if (version.libraries != null) {
            for (LibraryInfo library : version.libraries) {
                LibraryInfo libraryInfo = manifest.getLibrary(
                    library.name,
                    library.version
                );
                ...
                if (
                    !libsController.isInstalled(libraryInfo)
                ) libsController.install(libraryInfo, null);
            }
        }

        if (version.libsCollections != null) installLibraries(version, null);
        ...
        if (!versionDir.mkdirs()) {
            ...
            uiProvider
                ...
                .warning(
                    locale.get("installationError"),
                    locale.get("versionDirectoryCreationFailed")
                );
            return;
        }
        ...
        String fileUrl =
            version.download == null
                ? this.components.get(ManifestsURLs.class).getBaseDataURL() +
                  "/versions/" +
                  StringUtils.urlEncode(version.name) +
                  "/" +
                  StringUtils.urlEncode(version.name) +
                  "." +
                  (version.singleJar ? "jar" : "zip")
        ...
        try {
            ...
            LOG.logp(
                Level.SEVERE,
                "io.github.spookylauncher.VersionsInstaller",
                "install",
                "Throw!",
                e
            );
        ...
        Downloader.IDownloadMethod method = version.singleJar
            ? downloader::download
            : downloader::downloadAndUnpackZip;
        method.download(
            version.singleJar
                ? new File(versionDir, "minecraft.jar")
                : versionDir,
            urlCollector,
            options,
            success -> {
                ...
                if (success) {
                    ...
                    uiProvider
                        .panel()
                        .getButton(TitlePanel.PLAY)
                        .setText(locale.get("play"));
                }
            }
        );
    }
