...
public final class ComponentsRegister {
    ...
    private void createComponents(
        ...
        int jreController = controller.put(
            new JREController(
                controller,
                new File(launcherDirectory, "java"),
                JREsManifestDownloaderName
            )
        );
        int libsController = controller.put(
            new LibrariesController(
                controller,
                new File(workDirectory, "libraries"),
                librariesManifestDownloaderName
            )
        );
        int logsController = controller.put(
            new LogsController(controller, launcherDirectory)
        );
        int optionsController = controller.put(
            new OptionsController(
                controller,
                new File(launcherDirectory, "options.json")
            )
        );
        int versionsList = controller.put(
            new VersionsList(
                controller,
                new File(this.workDirectory, "versions"),
                manifestsURLs.getVersionsManifestURL()
            )
        );
        int versionsInstaller = controller.put(
            new VersionsInstaller(
                controller,
                librariesManifestDownloaderName,
                mirrorsManifestDownloaderName
            )
        );
        int discordPresenceViewer = controller.put(
            new DiscordPresenceViewer(controller)
        );
        int gameLauncher = controller.put(
            new GameLauncher(
                controller,
                librariesManifestDownloaderName,
                this.workDirectory
        ...
        try {
            ...
            uiProviderInstance = (LauncherComponent) constr.newInstance(
                controller,
                JREsManifestDownloaderName
            );
        } catch (Exception e) {
        ...
        int launcherManifestDownloader = controller.put(
            launcherManifestDownloaderName,
            new ManifestDownloader<>(
                controller,
                LauncherManifest.class,
                manifestsURLs.getLauncherManifestURL()
            )
        );
        int librariesManifestDownloader = controller.put(
            librariesManifestDownloaderName,
            new ManifestDownloader<>(
                controller,
                LibrariesManifest.class,
                manifestsURLs.getLibrariesManifestURL()
            )
        );
        int JREsManifestDownloader = controller.put(
            JREsManifestDownloaderName,
            new ManifestDownloader<>(
                controller,
                JREsManifest.class,
                manifestsURLs.getJREsManifestURL()
            )
        );
        int mirrorsManifestDownloader = controller.put(
            mirrorsManifestDownloaderName,
            new ManifestDownloader<>(
                controller,
                MirrorsManifest.class,
                manifestsURLs.getMirrorsManifestURL()
            )
        );
        ...
        Runtime.getRuntime().addShutdownHook(
            new Thread(() ->
                eventsManager.emitAndUnsubscribeAll(Events.SHUTDOWN)
            )
        );
    }
