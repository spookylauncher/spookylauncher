...
public final class VMArgumentsSelector {
    ...
    public String getVMArgs(
        Map<String, String> specificVmArgsMap,
        int currentJavaMajorVersion
    ) {
        String vmArgs = null;
        ...
        for (Map.Entry<String, String> entry : specificVmArgsMap.entrySet()) {
            ...
            int compareTypeEndIndex = StringUtils.find(
                rawKey,
                Character::isDigit
            );
            ...
            int majorVersionInCondition = Integer.parseInt(
                rawKey.substring(compareTypeEndIndex + 1)
            );

            if (
                NumbersComparator.compare(
                    compareType,
                    currentJavaMajorVersion,
                    majorVersionInCondition
                )
