...
public final class MessagesFactory {
    ...
    private static final HashMap<
        Integer,
        Class<? extends MappedBusMessage>
    > messagesClassesMap = new HashMap<>();
    private static final HashMap<
        Integer,
        MappedBusMessage
    > messagesInstancesMap = new HashMap<>();

    public static <T extends MappedBusMessage> T getOrCreateNewInstance(
        int type
    ) {
        MappedBusMessage msg = messagesInstancesMap.get(type);

        if (msg == null) {
            Class<? extends MappedBusMessage> clazz = messagesClassesMap.get(
                type
            );
