...
public class GameLauncher extends LauncherComponent {
    ...
    private final VMArgumentsSelector vmArgumentsSelector =
        new VMArgumentsSelector();

    public GameLauncher(
        ComponentsController components,
        String manifestDownloaderName,
        File workDirectory
    ) {
        ...
        if (
            !this.logsDirectory.exists() && !this.logsDirectory.mkdirs()
        ) LOG.severe("failed to create game logs directory!");
    }
    ...
    public void launch(final VersionInfo version) {
        ...
        final Options options = components
            .get(OptionsController.class)
            .getOptions();
        final LogsController logs = components.get(LogsController.class);
        final VersionsInstaller versionsInstaller = components.get(
            VersionsInstaller.class
        );
        ...
        if (options.selectedJavaType == null) {
            ...
            uiProvider
                .messages()
                .error(locale.get("error"), locale.get("noJre"));
            return;
        }
        ...
        uiProvider
            .panel()
            .setEnabledButtons(false, TitlePanel.VERSIONS, TitlePanel.PLAY);

        final Runnable launchTask = () -> {
            boolean success = startProcess(
                version,
                new File(
            ...
            if (success) {
                ...
                if (options.discordPresence) components
                    .get(DiscordPresenceViewer.class)
                    .showGamePresence(version);
            }
        };
    ...
    private List<LibraryInfo> unpackLibraries(
        LibrariesManifest manifest,
        LibrariesCollection collection
    ) {
        List<LibraryInfo> list = new ArrayList<>();

        for (LibraryInfo libInfo : collection.libraries)
            list.add(manifest.getLibrary(libInfo.name, libInfo.version));

        for (String includedCollectionName : collection.collections) {
            LibrariesCollection includedCollection = manifest.getLibsCollection(
                includedCollectionName
            );
    ...
    private boolean startProcess(
        ...
        final LibrariesController libsController = components.get(
            LibrariesController.class
        );
        final LibrariesManifest manifest = (
            (ManifestDownloader<LibrariesManifest>) components.get(
                manifestDownloaderName
            )
        ).getManifest();
        ...
        if (version.libsCollections != null) {
            for (String libs : version.libsCollections) {
                LibrariesCollection libsCollection = manifest.getLibsCollection(
                    libs
                );
        ...
        if (version.libraries != null) {
            for (LibraryInfo library : version.libraries)
                libraries.add(
                    manifest.getLibrary(library.name, library.version)
                );
        }

        libraries.addAll(LocalLibraries.LOCAL_LIBS);

        final String relativePathToLibrariesPrefix =
            "../../" +
            (IOUtils.getRelativePath(
                    workDirectory,
                    libsController.getLibrariesDirectory()
                )) +
            "/";
        ...
        for (LibraryInfo library : libraries) {
            ...
            if (library.isNative) {
                libraryPathBuilder
                    .append(relativePathToLibrariesPrefix)
                    .append(
                        IOUtils.getRelativePath(
                            libsController.getLibrariesDirectory(),
                            libFile
                ...
                libraryPathBuilder.append(';');
                specialLibraryPath.add(
                    "-D" + library.packet + ".librarypath=" + path
                );
            } else {
        ...
        if (libraryPathBuilder.length() > 0) libraryPathBuilder.setLength(
            libraryPathBuilder.length() - 1
        );

        classPathBuilder.append(
            version.launchData.getOrDefault("classPath", "minecraft.jar")
        );
        //classPathBuilder.append(';');
        //classPathBuilder.append("");
        ...
        final String jrePath = jreController
            .getJreExecutable(selectedJre)
            .getAbsolutePath();
        ...
        if (version.launchData.containsKey("vmArgs")) vmArgs =
            version.getLaunchProperty("vmArgs");
        else if (version.launchData.containsKey("specificVmArgs")) {
            String specificVmArgs = vmArgumentsSelector.getVMArgs(
                version.getLaunchProperty("specificVmArgs"),
                selectedJre.majorVersion
            );
        ...
        properties.setProperty(
            "main",
            version.getLaunchProperty("main", Constants.DEFAULT_MC_MAIN)
        );
        properties.setProperty(
            "launcher.dir",
        ...
        properties.setProperty(
            "launcher.resourcesDir",
            "\"" + workDirectory.getAbsolutePath() + "/resources\""
        );

        String gameArgs = Constants.DEFAULT_GAME_ARGS;

        if (version.hasProperty("gameArgs")) gameArgs +=
            " " + version.getLaunchProperty("gameArgs");

        for (String name : properties.stringPropertyNames())
            gameArgs = gameArgs.replace(
                "${" + name + "}",
                properties.getProperty(name)
            );

        if (!gameArgs.isEmpty()) cmd.addAll(
            CommandTokenizer.tokenize(gameArgs)
        );
        ...
        try {
            ...
            LOG.logp(
                Level.SEVERE,
                "io.github.spookylauncher.components.launch.GameLauncher",
                "startProcess",
                "THROW",
                e
            );
