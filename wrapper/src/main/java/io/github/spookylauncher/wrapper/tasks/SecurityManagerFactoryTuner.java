...
public final class SecurityManagerFactoryTuner {
    ...
    public static final String LOG_ID =
        SLWrapper.LOG_ID + "/security manager factory tuner";

    public boolean setup(Properties config, SecurityManagerFactory factory) {
        if (config.containsKey("securityRules")) {
            SecurityRules rules = Json.fromJson(
                config.getProperty("securityRules"),
                SecurityRules.class
            );
    ...
    public boolean setup(
        ...
        try {
            ...
            factory.setPermissionsCheckMode(
                actionToCheckMode(rules.actions.permissions)
            );
            factory.setPermissions(rules.permissions);

            factory.setExecuteCheckMode(
                actionToCheckMode(rules.actions.execute)
            );

            for (String executable : rules.executables)
                factory.addExecutable(
                    new File(pasteProperties(config, executable))
                );

            factory.setNativeLinkCheckMode(
                actionToCheckMode(rules.actions.nativeLink)
            );
            ...
            factory.setNetworkCheckMode(
                actionToCheckMode(rules.actions.network)
            );
    ...
    private CheckMode actionToCheckMode(SecurityActionType action) {
        return action == null
            ? null
            : (action == SecurityActionType.PERMIT
                  ? CheckMode.DENY_ALL_EXCEPT
                  : CheckMode.PERMIT_ALL_EXCEPT);
    }

    private void setAccesses(
        Properties config,
        HashMap<String, String> pathsAndMasks,
        BiConsumer<Integer, File> setAccessConsumer
    ) {
        for (Map.Entry<String, String> file : pathsAndMasks.entrySet()) {
            setAccessConsumer.accept(
                FileAccessType.getMaskFromFilePermissionActions(
                    file.getValue()
                ),
