...
public final class FilesChecker extends Checker {
    ...
    public FilesChecker(
        ...
        this.files = Collections.unmodifiableMap(
            new LinkedHashMap<>(toPaths(files))
        );
        this.directories = Collections.unmodifiableMap(
            new LinkedHashMap<>(toPaths(directories))
        );
    }
    ...
    public void checkAccess(String file, int accessType) {
        if (
            (mode == CheckMode.DENY_ALL_EXCEPT) ==
            (!files.containsKey(file) || (files.get(file) & accessType) == 0)
        ) {
            ...
            if (parents.size() > 0) {
                ...
                for (String parent : parents) {
                    if (
                        (mode == CheckMode.DENY_ALL_EXCEPT) ==
                        ((directories.get(parent) & accessType) != 0)
                    ) return;
                }
            }
            ...
            throw new SecurityException(
                "\"" +
                    FileAccessType.getName(accessType) +
                    "\" access type is not permitted for file \"" +
                    file +
                    "\""
            );
        }
