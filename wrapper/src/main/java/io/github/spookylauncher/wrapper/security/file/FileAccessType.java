...
public final class FileAccessType {
    ...
    public static final int READ = 0b1,
        WRITE = 0b10,
        DELETE = 0b100,
        EXECUTE = 0b1000,
        READLINK = 0b10000;
    ...
    public static int getMaskFromFilePermissionActions(String actions) {
        ...
        String[] names = actions.contains(",")
            ? actions.split(",")
            : new String[] { actions };

        int mask = 0;

        for (String name : names) mask = mask | getMask(name);
    ...
    public static boolean canExecute(int access) {
        return (access & EXECUTE) != 0;
    }

    public static boolean canReadlink(int access) {
        return (access & READLINK) != 0;
    }
}
