        ...
        try {
            ...
            if (
                Boolean.parseBoolean(
                    config.getProperty("useIsolation", "false")
                )
            ) {
                SecurityManagerFactory factory = new SecurityManagerFactory();

                SecurityManagerFactoryTuner tuner =
                    new SecurityManagerFactoryTuner();

                if (!tuner.setup(config, factory)) error = true;
                else {
                    factory.addIgnoredPackage(
            ...
            if (!error) {
                ...
                proxyClassLoader.closePackage(
                    "io.github.spookylauncher.spookylauncher"
                );
                ...
                if (!mcSetupTask.setup(proxyClassLoader, config)) {
                    ...
                    File resourcesDirectory = new File(
                        Objects.requireNonNull(
                            config.getProperty("resourcesDirectory"),
                            "resourcesDirectory property is not specified"
                        )
                    );

                    String downloadResClass = config.getProperty(
                        "tdrClass",
                        "net.minecraft.src.ThreadDownloadResources"
                    );

                    if (
                        !ClassUtils.classExists(
                            SLWrapper.class,
                            downloadResClass
                        )
                    ) {
            ...
            if (error) {
                Logger.log(
                    Level.FATAL,
                    LOG_ID,
                    "failed to launch minecraft from SL Wrapper"
                );
                System.exit(1);
            }
        }
