...
public final class MinecraftClassSetup {
    ...
    public static final String LOG_ID =
        SLWrapper.LOG_ID + "/minecraft class setup";
    ...
    public boolean setup(ProxyClassLoader proxyClassLoader, Properties config) {
        String minecraftClassName = config.getProperty(
            "minecraftClass",
        ...
        try {
            ...
            log(
                FATAL,
                LOG_ID,
                "minecraft class (" + minecraftClassName + ") not found"
            );
            log(FATAL, LOG_ID, e);
        } catch (Exception e) {
            log(
                FATAL,
                LOG_ID,
                "failed to load minecraft class (" + minecraftClassName + ")"
            );
            log(FATAL, LOG_ID, e);
        }
        ...
        if (config.containsKey("mcDirFieldIndex")) mcDirFields =
            Collections.singletonList(
                Integer.parseInt(config.getProperty("mcDirFieldIndex"))
            );
        ...
        for (int field : mcDirFields) {
            if (
                !changeMCDirectory(minecraftClass, field, directory)
            ) return false;
        }
    ...
    private boolean changeMCDirectory(
        Class<?> clazz,
        int fieldIndex,
        File directory
    ) {
