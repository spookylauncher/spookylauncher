...
public final class TDRTransformer implements ClassTransformer {
    ...
    > METHODS_TRANSFORMERS = new HashMap<>();
    ...
    static {
        METHODS_TRANSFORMERS.put(
            "<init>",
            TDRTransformer::transformConstructor
        );
        ...
        if (instruction instanceof TypeInsnNode) {
            ...
            if (
                ...
                if ("resources/".equals(filePathNode.cst)) {
                    MethodInsnNode fileCreatingNode =
                        (MethodInsnNode) visitor.peek(4);
    ...
    public void transform(ClassNode node) {
        ...
        for (MethodNode method : node.methods) {
            if (METHODS_TRANSFORMERS.containsKey(method.name)) {
                ...
                InstructionsTransformer processor = METHODS_TRANSFORMERS.get(
                    method.name
                );
