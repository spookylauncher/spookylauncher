...
final class PlayEvent extends Event {
    ...
    public void run() {
        ...
        if (!versions.isInstalled(selectedVersion)) installer.install(
            selectedVersion
        );
        else {
            JREsManifest manifest = (
                (ManifestDownloader<JREsManifest>) components.get(
                    manifestDownloaderName
                )
            ).getManifest();

            JreInfo jre = manifest.findJreInfoByMajorVersion(
                selectedVersion.javaMajorVersion
            );

            if (jre != null) {
                JREController jreController = components.get(
                    JREController.class
                );
                ...
                if (jreController.isInstalled(jre)) {
                    if (!jreController.isSelected(jre)) {
                        dialogs.showConfirmDialog(
                            ...
                            String.format(
                                locale.get("anotherJreRecommended"),
                                jre.majorVersion
                            )
                        );
                    }
                    ...
                    dialogs.showConfirmDialog(
                        () ->
                            jreController.installJre(jre, success -> {
                                if (success) {
                                    messages.info(
                                        locale.get("successfullyInstalled"),
                                        String.format(
                                            locale.get(
                                                "jreSuccessfullyInstalled"
                                            ),
                                            jre.majorVersion
                                        )
                                    );
                        ...
                        locale.get("poorJreCompatibility"),
                        String.format(
                            locale.get("anotherJreInstallRecommended"),
                            jre.majorVersion
                        ),
                        () -> launcher.launch(selectedVersion)
                    );
