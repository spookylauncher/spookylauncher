...
class DialogsImpl extends LauncherComponent implements Dialogs {
    ...
    public Tuple3<Label, ProgressBar, Dialog> createProgressDialog(
        ...
        Tuple3<JLabel, JProgressBar, JDialog> tuple = createSwingProgressDialog(
            title
        );
    ...
    public Tuple3<Label, ProgressBar, Runnable> createProgressView() {
        Tuple3<JLabel, JProgressBar, Runnable> tuple =
            createSwingProgressView();
    ...
    private Tuple3<JLabel, JProgressBar, Runnable> createSwingProgressView() {
        ...
        return new Tuple3<>(panel.label, panel.progressBar, () ->
            getMultiProgressDialog().removePanel(panel.panel)
        );
    }

    private Tuple3<JLabel, JProgressBar, JDialog> createSwingProgressDialog(
        String title
    ) {
        ProgressPanel panel = new ProgressPanel();
    ...
    public InstallAdapter createInstallationView(
        ...
        final boolean hasTitleConsumer =
            validAdapter && inputAdapter.titleConsumer != null;
        final boolean hasCancelSupplier =
            validAdapter && inputAdapter.cancelSupplier != null;
        final boolean hasProgressConsumer =
            validAdapter && inputAdapter.progressConsumer != null;
        ...
        adapter.cancelSupplier = () ->
            (hasCancelSupplier && inputAdapter.cancelSupplier.get()) ||
            abortInstall.get();
        ...
        adapter.onCancel = () -> {
            if (
                validAdapter && inputAdapter.onCancel != null
            ) inputAdapter.onCancel.run();
        };

        adapter.progressConsumer = progress -> {
            tuple.y.setValue(progress);

            if (hasProgressConsumer) inputAdapter.progressConsumer.accept(
                progress
            );
        };
