...
public final class AssetsLoader {
    ...
    // { font-path, font-size }
    private static final String[] FONTS = { "Minecraftia-Regular.ttf" };
    ...
    public static void load() {
        for (String fontPath : FONTS) {
            try {
                ...
                LOG.info(
                    String.format(
                        "font \"%s\" successfully loaded and registered",
                        font.getName()
                    )
                );
            } catch (Exception e) {
                ...
                LOG.logp(
                    Level.SEVERE,
                    "io.github.spookylauncher.components.ui.swing.FontLoader",
                    "loadFonts",
                    "THROW",
                    e
                );
