...
public final class Bootstrap implements Runnable {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (!launcherInternalDir.exists() && !launcherInternalDir.mkdirs()) {
            LOG.severe(
                "failed to create internal launcher directory (" +
                    launcherInternalDir.getAbsolutePath() +
                    ")"
            );
            System.exit(1);
            return;
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("--viaprotocol")) {
            ...
            try {
                protocolConsumer = ProtocolRequestHandler.createConsumer(
                    args[1],
                    hasActiveInstance
                );
        ...
        new Bootstrap(
            ...
            protocolConsumer,
            hasActiveInstance,
            hasActiveInstance ? null : IOUtils.lock(lockFile)
        ).run();
    }

    public Bootstrap(
        final File workDirectory,
        final Consumer<ComponentsController> protocolConsumer,
        final boolean hasActiveInstance,
        final FileLock lock
    ) {
    ...
    @Override
    public void run() {
        final boolean workDirectoryCreationFailed =
            !this.workDirectory.exists() && !this.workDirectory.mkdirs();

        final ComponentsController controller = new ComponentsController();

        final ComponentsRegister reg = new ComponentsRegister(
            controller,
            this.workDirectory
        );
        ...
        if (this.protocolConsumer != null) this.protocolConsumer.accept(
            controller
        );
        ...
        if (workDirectoryCreationFailed) LOG.info(
        ...
        if (protocolConsumer == null && this.hasActiveInstance) {
            ...
            controller.addOnInitializedEvent(
                controller.getIndex(translator),
                c -> {
                    ...
                    JOptionPane.showMessageDialog(
                        null,
                        locale.get("launcherAlreadyRunning"),
                        locale.get("startError"),
        ...
        controller
            ...
            .subscribe(Events.SHUTDOWN, args -> {
        ...
        if (
            protocolConsumer == null &&
            !controller.isInitialized(UIProvider.class)
        ) {
